...
include_once("includes/connection.php");
	...
	if(isset($_POST['submit']) && !isset($_SESSION['logInUser'])) {
		...
		if($em[1]=="somaiya.edu" && $flag==1) {
			$eml=$_POST['email'];
			$query="SELECT * FROM admindet WHERE email='$eml'";
			...
			if(mysqli_num_rows($res) > 0) {
				...
				$qu = "UPDATE admindet SET OTP = '$otpgen', expiry_time = '$t' WHERE email = '".$_SESSION['logInUser']."' ";
		...
		$qu = "SELECT * FROM admindet where email = '".$_SESSION['logInUser']."'";
		...
		if(mysqli_num_rows($res)>0) {
			...
			if($exp > $t ) {
				$qu = "UPDATE admindet SET OTP = '', expiry_time = '' WHERE email = '".$_SESSION['logInUser']."' ";
				if(!mysqli_query($conn, $qu))
					echo mysqli_error;
				else {
