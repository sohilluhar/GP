...
include_once("includes/connection.php");
	...
	if(isset($_POST['submit']) && !isset($_SESSION['logInUser'])) {
		...
		if($em[1]=="somaiya.edu" && $flag==1) {
			$eml=$_POST['email'];
			if($eml == 'principal.engg@somaiya.edu') {
				$_SESSION['logInUser'] = $eml;
				$namerow = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM commdet WHERE email = '$eml'"));
				$name = $namerow['name'];
				$_SESSION['name'] = $name;
			}
			else {
				$query="SELECT * FROM commdet WHERE email='$eml' ";
			...
			if(isset($_SESSION['logInUser'])) {
				...
				if($_SESSION['logInUser'] == 'principal.engg@somaiya.edu') {
					$qu = "UPDATE commdet SET OTP = '$otpgen', expiry_time = '$t' WHERE email = '".$_SESSION['logInUser']."' ";
				}
				else {
					$qu = "UPDATE commdet SET OTP = '$otpgen', expiry_time = '$t' WHERE email = '".$_SESSION['logInUser']."' ";
				}
				if(!mysqli_query($conn, $qu))
					...
					Please verify by entering the following One Time Password(OTP): <b>".$otpgen."</b>";
					try {
						//Server settings
				...
				$q = "INSERT INTO logins VALUES ('".$_SESSION['logInUser']."', 'Failed authority login attempt from an invalid Somaiya ID', CURRENT_TIMESTAMP)";
				$r = mysqli_query($conn, $q);
				$_SESSION['crederr'] = "<h5><font color='red'>The entered Email ID is not linked to a valid authority. Please try logging in again.</font></h5>";
				$_SESSION['emailerr'] = "";
			}
		}
		
		else if ($em[1]!="somaiya.edu" && $flag == 1){
			$q = "INSERT INTO logins VALUES ('".$_SESSION['logInUser']."', 'Failed authority intrusion from a Non-Somaiya ID', CURRENT_TIMESTAMP)";
			$r = mysqli_query($conn, $q);
			$_SESSION['emailerr'] = "<h5><font color='red'>Login with Somaiya Email ID only!</font></h5>";
			$_SESSION['crederr'] = "";
		}
		...
		if($_SESSION['logInUser'] == 'principal.engg@somaiya.edu') {
			$qu = "SELECT * FROM commdet where email = '".$_SESSION['logInUser']."'";
		}
		else {
			$qu = "SELECT * FROM commdet where email = '".$_SESSION['logInUser']."'";
		}
		...
		if(mysqli_num_rows($res)>0) {
			...
			if($exp > $t) {
				...
				if($_SESSION['logInUser'] == 'principal.engg@somaiya.edu') {
					$qu = "UPDATE commdet SET OTP = '', expiry_time = '' WHERE email = '".$_SESSION['logInUser']."' ";
				}
				else {
					$qu = "UPDATE commdet SET OTP = '', expiry_time = '' WHERE email = '".$_SESSION['logInUser']."' ";
				}
				...
				if(!mysqli_query($conn, $qu))
					...
					$q = "INSERT INTO logins VALUES ('".$_SESSION['logInUser']."', 'Successful Authority Login', CURRENT_TIMESTAMP)";
					$r = mysqli_query($conn, $q);
					if($_SESSION['logInUser']=='principal.engg@somaiya.edu') {
